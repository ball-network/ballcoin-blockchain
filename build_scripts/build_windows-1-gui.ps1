...
git submodule update --init ballcoin-blockchain-gui
...
# Other than `ballcoin-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @ball-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@ball-network` because it causes an error on later `electron-packager` command
Remove-Item "@ball-network" -Recurse -Force
