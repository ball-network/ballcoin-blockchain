...
if [ ! "$BALL_INSTALLER_VERSION" ]; then
...
pip install j2cli
CLI_DEB_BASE="ballcoin-blockchain-cli_$BALL_INSTALLER_VERSION-1_$PLATFORM"
mkdir -p "dist/$CLI_DEB_BASE/opt/ball"
...
# sets the version for ballcoin-blockchain in package.json
cp package.json package.json.orig
jq --arg VER "$BALL_INSTALLER_VERSION" '.version=$VER' package.json > temp.json && mv temp.json package.json
...
GUI_DEB_NAME=ballcoin-blockchain_${BALL_INSTALLER_VERSION}_${PLATFORM}.deb
mv "dist/${PRODUCT_NAME}-${BALL_INSTALLER_VERSION}.deb" "../../../build_scripts/dist/${GUI_DEB_NAME}"
