...
from ball.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg, get_wallet_client
...
from ball.wallet.util.query_filter import TransactionTypeFilter, HashFilter
...
def print_transaction(
    ...
    if verbose:
        ...
        ball_amount = Decimal(int(tx.amount)) / units["ball"]
        ...
        print(f"Amount {description}: {ball_amount} {address_prefix.upper()}")
...
async def stake_info(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        if is_stake_farm:
            print(f"Stake Balance: {response['balance'] / units['ball']}")
            print(f"Stake Balance Other: {response['balance_other'] / units['ball']}")
            print(f"Stake Balance Income: {response['balance_income'] / units['ball']}")
            print(f"Stake Balance 24H Exp: {response['balance_exp'] / units['ball']}")
            print(f"Stake Reward: {response['stake_reward'] / units['ball']}")
...
async def stake_send(
    ...
    if stake_type is not None:
        ...
        for i, value in enumerate(value_list):
            ...
            print(
                ...
                f"({value.coefficient}"
                f")" if is_stake_farm else f",{str(float(Decimal(value.reward_coefficient) * 10000))+'‱'})"
            )
        val = None
        prompt: str = (
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        final_amount = int(amount * units["ball"])
        if not (final_amount / units["ball"]).is_integer():
            print(
...
#             return
#         response = await wallet_client.spend_stake_coins(tx_ids, int(fee * units["ball"]))
#         print(str(response))
