...
from ball.util.db_version import set_db_version
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("ball_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("ball_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("ball_target_address") or all_targets[-2] == config["pool"].get(
                    "ball_target_address"
                ):
        ...
        if "ball_target_address" not in config["farmer"]:
            print(
                f"Setting the ball destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
        ...
        if "ball_target_address" not in config["timelord"]:
            print(
                f"Setting the chiax destination for the timelord fee (1/200 plus fees)"
                f" to {all_targets[0]}"
            )
...
def create_all_ssl(
    ...
    *,
    ...
    write_ssl_cert_and_key(ball_ca_crt_path, ball_ca_crt, ball_ca_key_path, ball_ca_key, overwrite=overwrite)
...
def init(
    ...
    if create_certs is not None:
        ...
        return ball_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def ball_init(
    ...
    *,
    ...
    """
    ball_root = os.environ.get("BALL_ROOT", None)
