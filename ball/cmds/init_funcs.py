...
from ball.ssl.create_ssl import (
    ensure_ssl_dirs,
...
from ball.util.path import mkdir
from ball.util.ssl_check import (
...
from ball.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
from ball.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'ball keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "ball_target_address" not in config["farmer"]
    stop_searching_for_pool = "ball_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("ball_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("ball_target_address"):
                stop_searching_for_pool = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "ball_target_address" not in config["farmer"]:
        print(
            f"Setting the ball destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["ball_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["ball_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['ball_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "ball_target_address" not in config["pool"]:
        print(f"Setting the ball destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["ball_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["ball_target_address"] not in all_targets:
        print(
    ...
    if "ball_target_address" not in config["timelord"]:
        print(
            f"Setting the ball destination for the timelord fee (2/100 plus fees)"
            f" to {all_targets[0]}"
        )
...
def create_all_ssl(root_path: Path):
    ...
    ball_ca_crt, ball_ca_key = get_ball_ca_crt_key()
    ball_ca_crt_path = ca_dir / "ball_ca.crt"
    ball_ca_key_path = ca_dir / "ball_ca.key"
    write_ssl_cert_and_key(ball_ca_crt_path, ball_ca_crt, ball_ca_key_path, ball_ca_key)
    ...
    ball_ca_crt, ball_ca_key = get_ball_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, ball_ca_crt, ball_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                ball_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return ball_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)


def ball_version_number() -> Tuple[str, str, str, str]:
...
def ball_minor_release_number():
    res = int(ball_version_number()[2])
    print(f"Install release number: {res}")
    return res


def ball_full_version_str() -> str:
    major, minor, patch, dev = ball_version_number()
...
def ball_init(
    ...
    """
    if os.environ.get("BALL_ROOT", None) is not None:
        print(
            f"warning, your BALL_ROOT is set to {os.environ['BALL_ROOT']}. "
            f"Please unset the environment variable and run ball init again\n"
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'ball keys show --show-mnemonic-seed'")
