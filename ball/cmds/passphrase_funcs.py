...
from ball.daemon.client import acquire_connection_to_daemon
from ball.util.config import load_config
from ball.util.errors import KeychainMaxUnlockAttempts
from ball.util.keychain import Keychain, supports_os_passphrase_storage
from ball.util.keyring_wrapper import KeyringWrapper, DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
from ball.util.misc import prompt_yes_no
from ball.util.ws_message import WsRpcMessage
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other Ball processes will be able to access your keys without prompting for your passphrase."
            )
...
def read_passphrase_from_file(passphrase_file: TextIOWrapper) -> str:
    passphrase = passphrase_file.read().rstrip(os.environ.get("BALL_PASSPHRASE_STRIP_TRAILING_CHARS", "\r\n"))
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'ball passphrase set' or 'ball passphrase remove' to update or remove your passphrase")
        sys.exit(1)
