...
from ball.cmds.passphrase_funcs import obtain_current_passphrase
from ball.daemon.client import connect_to_daemon_and_validate
...
from ball.util.errors import KeychainException
from ball.util.file_keyring import MAX_LABEL_LENGTH
from ball.util.ints import uint32
from ball.util.keychain import Keychain, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed
from ball.util.keyring_wrapper import KeyringWrapper
...
def generate_and_print():
    ...
    print(mnemonic)
    print("Note that this key has not been added to the keychain. Run ball keys add")
    return mnemonic
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'ball keys generate'")
...
def derive_sk_from_hd_path(master_sk: PrivateKey, hd_path_root: str) -> Tuple[PrivateKey, str]:
    ...
    from ball.wallet.derive_keys import _derive_path, _derive_path_unhardened
...
async def migrate_keys(root_path: Path, forced: bool = False) -> bool:
    ...
    deprecation_message = (
        ...
        "You need to migrate your keyring to continue using Ball.\n"
    )
