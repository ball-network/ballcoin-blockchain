...
from ball.util.misc import format_bytes
from ball.util.misc import format_minutes
from ball.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total ball farmed: {amounts['farmed_amount'] / units['ball']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['ball']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['ball']}")
            ...
            async with get_any_service_client("full_node", rpc_port) as (client, config, _):
                if client is not None:
                    ...
                    for k, v in sorted(PlotStats.staking_ph.items(), key=(lambda tup: tup[1]), reverse=True):
                        ...
                        balance = sum(coin.coin.amount for coin in coins)
                        address = encode_puzzle_hash(puzzle_hash, address_prefix)
                        print(f"  {address} (balance: {balance / units['ball']}, plots: {v})")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'ball start wallet' and 'ball wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'ball wallet show'")
    else:
        print("Note: log into your key using 'ball wallet show' to see rewards for each key")
