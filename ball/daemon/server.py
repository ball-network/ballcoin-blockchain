...
from ball.cmds.init_funcs import check_keys, ball_init, ball_full_version_str
...
from ball.util.errors import KeychainRequiresMigration, KeychainCurrentPassphraseIsInvalid
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"ball-blockchain version: {ball_full_version_str()}")
