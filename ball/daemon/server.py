...
from ball.cmds.init_funcs import check_keys, ball_full_version_str, ball_init
from ball.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from ball.daemon.keychain_server import KeychainServer, keychain_commands
from ball.daemon.windows_signal import kill
from ball.plotters.plotters import get_available_plotters
from ball.plotting.util import add_plot_directory
from ball.server.server import ssl_context_for_root, ssl_context_for_server
from ball.ssl.create_ssl import get_mozilla_ca_crt
from ball.util.beta_metrics import BetaMetricsLogger
...
from ball.util.json_util import dict_to_json_str
from ball.util.keychain import Keychain, passphrase_requirements, supports_os_passphrase_storage
from ball.util.lock import Lockfile, LockfileError
from ball.util.network import WebServer
from ball.util.service_groups import validate_service
from ball.util.setproctitle import setproctitle
from ball.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Ball.")
    quit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "ball": "ball",
        "ball_data_layer": "start_data_layer",
        "ball_data_layer_http": "start_data_layer_http",
        "ball_wallet": "start_wallet",
        "ball_full_node": "start_full_node",
        "ball_harvester": "start_harvester",
        "ball_farmer": "start_farmer",
        "ball_introducer": "start_introducer",
        "ball_timelord": "start_timelord",
        "ball_timelord_launcher": "timelord_launcher",
        "ball_full_node_simulator": "start_simulator",
        "ball_seeder": "start_seeder",
        "ball_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Ball will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["ball", "plotters", plotter]

        if plotter == "bladebit":
            # plotter command must be either
            # 'ball plotters bladebit ramplot' or 'ball plotters bladebit diskplot'
            plot_type = request["plot_type"]
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered BALL_ROOT
    os.environ["BALL_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with BALL_ROOT: {os.environ['BALL_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in ball_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    ball_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("ball_daemon")
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"ballcoin-blockchain version: {ball_full_version_str()}")
