...
from ball.consensus.block_rewards import calculate_base_farmer_reward
...
from ball.pools.pool_puzzles import create_p2_singleton_puzzle
...
from ball.types.signing_mode import SigningMode
...
from ball.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, SINGLETON_MOD_HASH
...
from ball.wallet.singleton import create_singleton_puzzle
...
from ball.wallet.util.compute_hints import compute_coin_hints
...
class WalletRpcApi:
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from ball.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        ball_coins = None
        if ball_coin_list:
            ball_coins = set([Coin.from_json_dict(ball_coin) for ball_coin in ball_coin_list])
        ...
        if mint_from_did:
            ...
            sb = await nft_wallet.mint_from_ball(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
    ...
    async def staking_send(self, request) -> EndpointResult:
        ...
        async with self.service.wallet_state_manager.lock:
            tx: TransactionRecord = await wallet.generate_signed_transaction(
                amount, puzzle_hash, uint64(0), memos=["staking".encode("utf-8")]
            )
            await wallet.push_transaction(tx)
        return {
            "transaction": tx.to_json_dict_convenience(self.service.config),
    ...
    async def find_pool_nft(self, request) -> EndpointResult:
        ...
        if self.service.logged_in_fingerprint is None:
            return {"error": "not wallet login"}
        total_amount = 0
        record_amount = 0
        try:
