...
from ball.consensus.block_rewards import calculate_base_farmer_reward, MOJO_PER_BALL
from ball.data_layer.data_layer_errors import LauncherCoinNotFoundError
from ball.data_layer.data_layer_wallet import DataLayerWallet
from ball.pools.pool_puzzles import create_p2_singleton_puzzle, POOL_OUTER_MOD_HASH
from ball.pools.pool_wallet import PoolWallet
from ball.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
from ball.protocols.protocol_message_types import ProtocolMessageTypes
from ball.protocols.wallet_protocol import CoinState
from ball.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
from ball.rpc.util import tx_endpoint
from ball.server.outbound_message import NodeType, make_msg
from ball.server.ws_connection import WSBallConnection
from ball.simulator.simulator_protocol import FarmNewBlockProtocol
from ball.types.announcement import Announcement
...
from ball.types.signing_mode import CHIP_0002_SIGN_MESSAGE_PREFIX, SigningMode
from ball.types.spend_bundle import SpendBundle, estimate_fees
from ball.types.stake_value import (
    STAKE_FARM_PREFIX,
    STAKE_FARM_LIST,
    STAKE_FARM_MIN,
    STAKE_FARM_COUNT,
...
from ball.util.config import load_config, str2bool
from ball.util.errors import KeychainIsLocked
from ball.util.ints import uint16, uint32, uint64, uint128
from ball.util.keychain import bytes_to_mnemonic, generate_mnemonic
from ball.util.misc import UInt32Range
from ball.util.path import path_from_root
from ball.util.streamable import Streamable, streamable
from ball.util.ws_message import WsRpcMessage, create_payload_dict
from ball.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from ball.wallet.cat_wallet.cat_info import CRCATInfo
from ball.wallet.cat_wallet.cat_wallet import CATWallet
from ball.wallet.cat_wallet.dao_cat_info import LockedCoinInfo
from ball.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from ball.wallet.conditions import Condition
from ball.wallet.dao_wallet.dao_info import DAORules
from ball.wallet.dao_wallet.dao_utils import (
...
from ball.wallet.dao_wallet.dao_wallet import DAOWallet
from ball.wallet.derive_keys import (
    MAX_POOL_WALLETS,
...
from ball.wallet.did_wallet import did_wallet_puzzles
from ball.wallet.did_wallet.did_info import DIDCoinData, DIDInfo
from ball.wallet.did_wallet.did_wallet import DIDWallet
from ball.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
    did_program_to_metadata,
    match_did_puzzle,
    metadata_to_program,
)
from ball.wallet.nft_wallet import nft_puzzles
from ball.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTInfo
from ball.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from ball.wallet.nft_wallet.nft_wallet import NFTWallet
from ball.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from ball.wallet.notification_store import Notification
from ball.wallet.outer_puzzles import AssetType
from ball.wallet.payment import Payment
from ball.wallet.puzzle_drivers import PuzzleInfo, Solver
from ball.wallet.puzzles.clawback.metadata import AutoClaimSettings, ClawbackMetadata
from ball.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from ball.wallet.puzzles.stake.metadata import AutoWithdrawStakeSettings
from ball.wallet.singleton import (
    SINGLETON_LAUNCHER_PUZZLE_HASH,
    create_singleton_puzzle,
    get_inner_puzzle_from_singleton,
)
from ball.wallet.trade_record import TradeRecord
from ball.wallet.trading.offer import Offer
from ball.wallet.transaction_record import TransactionRecord
from ball.wallet.uncurried_puzzle import uncurry_puzzle
from ball.wallet.util.address_type import AddressType, is_valid_address
from ball.wallet.util.compute_hints import compute_spend_hints_and_additions
from ball.wallet.util.compute_memos import compute_memos
from ball.wallet.util.puzzle_decorator_type import PuzzleDecoratorType
from ball.wallet.util.query_filter import HashFilter, TransactionTypeFilter
from ball.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from ball.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig
from ball.wallet.util.wallet_sync_utils import fetch_coin_spend_for_coin_state
from ball.wallet.util.wallet_types import CoinType, WalletType
from ball.wallet.vc_wallet.cr_cat_drivers import ProofsChecker
from ball.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from ball.wallet.vc_wallet.vc_store import VCProofs
from ball.wallet.vc_wallet.vc_wallet import VCWallet
from ball.wallet.wallet import Wallet
from ball.wallet.wallet_coin_record import WalletCoinRecord
from ball.wallet.wallet_coin_store import CoinRecordOrder, GetCoinRecords, unspent_range
from ball.wallet.wallet_info import WalletInfo
...
class WalletRpcApi:
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: WSBallConnection, coin_id: bytes32, latest: bool = True
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("ball_target_address")
        pool_target = config["pool"].get("ball_target_address")
    ...
    async def create_new_wallet(
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from ball.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def verify_signature(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        # signatures made by `ball keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def get_offer_summary(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        from ball.util.bech32m import bech32_decode, convertbits
        from ball.wallet.util.puzzle_compression import OFFER_MOD_OLD, decompress_object_with_puzzles
    ...
    async def cancel_offers(
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "ball")
        ...
        key = None
        if asset_id is not None and asset_id != "ball":
    ...
    async def did_get_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.BALL.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(
        ...
        ball_coin_list = request.get("ball_coins", None)
        ball_coins = None
        if ball_coin_list:
            ball_coins = {Coin.from_json_dict(ball_coin) for ball_coin in ball_coin_list}
        ball_change_target = request.get("ball_change_target", None)
        if ball_change_target is not None:
            if ball_change_target[:2] == "ball":
                ball_change_ph = decode_puzzle_hash(ball_change_target)
            else:
                ball_change_ph = bytes32(hexstr_to_bytes(ball_change_target))
        else:
            ball_change_ph = None
        ...
        if mint_from_did:
            txs = await nft_wallet.mint_from_did(
                ...
                ball_coins=ball_coins,
                ball_change_ph=ball_change_ph,
            ...
            txs = await nft_wallet.mint_from_ball(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
    ...
    async def stake_send(
        ...
        if is_stake_farm:
            if amount < STAKE_FARM_MIN:
                return {"success": False, "error": f"stake farm must more then {int(STAKE_FARM_MIN/MOJO_PER_BALL)} BALL"}
            if address != "":
            ...
            if amount < STAKE_LOCK_MIN:
                return {"success": False, "error": f"stake lock must more then {int(STAKE_LOCK_MIN/MOJO_PER_BALL)} BALL"}
            if address != "":
                selected_network = self.service.config["selected_network"]
                expected_prefix = self.service.config["network_overrides"]["config"][selected_network]["address_prefix"]
                if address[0: len(expected_prefix)] != expected_prefix:
