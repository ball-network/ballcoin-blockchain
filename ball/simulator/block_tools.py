...
from ball.consensus.block_rewards import calculate_base_farmer_reward, calculate_pool_reward, \
    calculate_community_reward, calculate_timelord_fee
from ball.consensus.blockchain_interface import BlockchainInterface
from ball.consensus.coinbase import create_farmer_coin, create_pool_coin, create_puzzlehash_for_pk, \
    create_community_coin, create_timelord_coin
from ball.consensus.condition_costs import ConditionCost
...
from ball.consensus.full_block_to_block_record import block_to_block_record
from ball.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from ball.consensus.pot_iterations import (
    calculate_ip_iters,
...
from ball.plotters.chiapos import Params
from ball.plotting.create_plots import PlotKeys, create_plots
from ball.plotting.manager import PlotManager
from ball.plotting.util import (
    PlotRefreshEvents,
    PlotRefreshResult,
...
from ball.server.server import ssl_context_for_client
from ball.simulator.socket import find_available_listen_port
from ball.simulator.ssl_certs import (
...
from ball.simulator.time_out_assert import time_out_assert_custom_interval
from ball.simulator.wallet_tools import WalletTool
from ball.ssl.create_ssl import create_all_ssl
...
from ball.types.blockchain_format.proof_of_space import (
    ProofOfSpace,
...
from ball.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from ball.types.blockchain_format.sized_bytes import bytes32
from ball.types.blockchain_format.slots import (
...
from ball.util.block_cache import BlockCache
from ball.util.config import (
    config_path_for_filename,
    create_default_ball_config,
    load_config,
    lock_config,
    override_config,
    save_config,
...
from ball.util.keychain import Keychain, bytes_to_mnemonic
from ball.util.prev_transaction_block import get_prev_transaction_block
from ball.util.vdf_prover import get_vdf_info_and_proof
...
from ball.wallet.puzzles.rom_bootstrap_generator import GENERATOR_MOD
...
class BlockTools:
    ...
    def __init__(
        ...
        if automated_testing:
            ...
            create_default_ball_config(root_path)
            create_all_ssl(
                root_path,
        ...
        try:
            if self.local_keychain:
                ...
                keychain_proxy = await connect_to_keychain_and_validate(
                    self.root_path, self.log, user="testing-1.8.0", service="ball-testing-1.8.0"
                )
            ...
            if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
                raise RuntimeError("Keys not generated. Run `ball keys generate`")
        ...
        try:
            if plot_keys is None:
                ...
                if pool_contract_puzzle_hash is None:
                    ...
                    pool_address = encode_puzzle_hash(pool_contract_puzzle_hash, "ball")
...
def get_signage_point(
    ...
    if signage_point_index == 0:
        return SignagePoint(None, None, None, None, constants.GENESIS_TIMELORD_PUZZLE_HASH)
    ...
    return SignagePoint(cc_sp_vdf, cc_sp_proof, rc_sp_vdf, rc_sp_proof, constants.GENESIS_TIMELORD_PUZZLE_HASH)
...
def create_test_foliage(
    ...
    Args:
        ...
        timelord_fee_puzzle_hash: where to pay out timelord reward
    ...
    if is_transaction_block:
        ...
        if height > 0:
            ...
            timelord_coin = create_timelord_coin(
                ...
                calculate_timelord_fee(curr.height),
                constants.GENESIS_CHALLENGE,
            )
...
def create_test_unfinished_block(
    ...
    if signage_point.cc_vdf is not None:
        ...
        signage_point = SignagePoint(None, None, None, None, constants.GENESIS_TIMELORD_PUZZLE_HASH)
