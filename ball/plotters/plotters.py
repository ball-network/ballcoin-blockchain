...
from ball.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in BALL_ROOT.
    ball_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within BALL_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
