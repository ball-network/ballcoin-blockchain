...
class Blockchain(BlockchainInterface):
    ...
    async def validate_unfinished_block(
    ...
    ) -> Tuple[PreValidationResult, Optional[str]]:
        ...
        return PreValidationResult(None, required_iters, cost_result, False), difficulty_coefficient
    ...
    async def get_farmer_difficulty_coefficient(
        ...
        staking = await self.get_peak_farmer_staking(farmer_public_key, peak_height)
        ...
        #             "address": encode_puzzle_hash(create_puzzlehash_for_pk(farmer_public_key), prefix),
        #             "blocks": blocks,
        ...
        #
        # log.info(
        #     f"height: {peak_height}, minimal_staking : {minimal_staking / units['greenbtc']}, "
        #     f"staking: {staking / units['greenbtc']}, difficulty_coefficient: {coefficient}, "
        #     f"space : {space / (1024 ** 4):.5f} TiB total space: {network_space / (1024 ** 4):.5f} TiB, "
        #     f"blocks: {blocks}, farmer_public_key: {farmer_public_key}"
        # )
        return coefficient

    async def get_peak_farmer_staking(self, farmer_public_key: G1Element, peak_height: Optional[uint32]) -> uint64:
        coins = await self.coin_store.get_unspent_coins_before_height(
            create_puzzlehash_for_pk(farmer_public_key), peak_height
        )
