...
from ball.wallet.trading.offer import (
    OFFER_MOD,
    OFFER_MOD_HASH,
    OFFER_MOD_OLD,
    OFFER_MOD_OLD_HASH,
...
from ball.wallet.util.wallet_sync_utils import fetch_coin_spend
from ball.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
from ball.wallet.wallet import CHIP_0002_SIGN_MESSAGE_PREFIX, Wallet
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSBallConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSBallConnection) -> None:
    ...
    async def mint_from_did(
        ...
        if ball_coins is None:
            ball_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        ...
        ball_primaries = [
            AmountWithPuzzlehash({"puzzlehash": ball_change_ph, "amount": change, "memos": [ball_change_ph]})
        ]
        ...
        for ball_coin in ball_coins:
            ...
            if first:
                ...
                message_list.append(
                    Coin(ball_coin.name(), ball_primaries[0]["puzzlehash"], ball_primaries[0]["amount"]).name()
                )
        ...
        ball_spend = await self.standard_wallet.sign_transaction(ball_spends)
    ...
    async def mint_from_ball(
        ...
        for ball_coin in ball_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=ball_primaries + primaries,
                    fee=fee,
