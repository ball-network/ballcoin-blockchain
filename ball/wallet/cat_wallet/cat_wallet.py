...
from ball.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        ball_tx = dataclasses.replace(ball_tx, spend_bundle=spend_bundle)
    ...
    async def create_tandem_ball_tx(
        ...
        if fee > amount_to_claim:
            ball_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            ...
            if first:
                ...
                if need_ball_transaction:
                    if fee > regular_ball_to_claim:
                        ...
                        ball_tx, _ = await self.create_tandem_ball_tx(
                            fee, uint64(regular_ball_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
