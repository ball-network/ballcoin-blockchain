...
from ball.types.coin_spend import compute_additions_with_cost
...
from ball.util.errors import Err, ValidationError
...
from ball.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, LegacyCATInfo
...
from ball.wallet.coin_selection import select_coins
from ball.wallet.conditions import Condition, ConditionValidTimes, UnknownCondition, parse_timelock_info
...
from ball.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
from ball.wallet.util.curry_and_treehash import calculate_hash_of_quoted_mod_hash, curry_and_treehash
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            ball_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        ball_tx = dataclasses.replace(ball_tx, spend_bundle=spend_bundle, name=spend_bundle.name())
        await self.standard_wallet.push_transaction(ball_tx)
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSBallConnection, parent_coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def create_tandem_ball_tx(
        ...
        if fee > amount_to_claim:
            ball_coins = await self.standard_wallet.select_coins(
                fee,
                tx_config.coin_selection_config,
            )
            origin_id = list(ball_coins)[0].name()
            ...
            origin_id = list(ball_coins)[0].name()
            selected_amount = sum([c.amount for c in ball_coins])
            [ball_tx] = await self.standard_wallet.generate_signed_transaction(
            ...
            message = None
            for spend in ball_tx.spend_bundle.coin_spends:
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some BALL as part of this
        regular_ball_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_ball_to_claim = payment_amount

        need_ball_transaction = (fee > 0 or regular_ball_to_claim > 0) and (fee - regular_ball_to_claim != 0)
        ...
        for coin in cat_coins:
            ...
            if first:
                ...
                if need_ball_transaction:
                    if fee > regular_ball_to_claim:
                        ball_tx, _ = await self.create_tandem_ball_tx(
                            fee,
                            uint64(regular_ball_to_claim),
                            tx_config,
                    ...
                    elif regular_ball_to_claim > fee:
                        ball_tx, _ = await self.create_tandem_ball_tx(
                            fee,
                            uint64(regular_ball_to_claim),
                            tx_config,
        ...
        ball_spend_bundle = SpendBundle([], G2Element())
        if ball_tx is not None and ball_tx.spend_bundle is not None:
            ball_spend_bundle = ball_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if ball_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=ball_tx.confirmed_at_height,
                    created_at_time=ball_tx.created_at_time,
                    to_puzzle_hash=ball_tx.to_puzzle_hash,
                    amount=ball_tx.amount,
                    fee_amount=ball_tx.fee_amount,
                    confirmed=ball_tx.confirmed,
                    sent=ball_tx.sent,
                    spend_bundle=None,
                    additions=ball_tx.additions,
                    removals=ball_tx.removals,
                    wallet_id=ball_tx.wallet_id,
                    sent_to=ball_tx.sent_to,
                    trade_id=ball_tx.trade_id,
                    type=ball_tx.type,
                    name=ball_tx.name,
                    memos=[],
                    valid_times=parse_timelock_info(extra_conditions),
