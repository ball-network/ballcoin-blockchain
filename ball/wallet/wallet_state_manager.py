...
from ball.consensus.block_rewards import (
    calculate_base_farmer_reward,
    calculate_pool_reward,
    calculate_stake_farm_reward,
)
from ball.consensus.coinbase import (
    farmer_parent_id,
    pool_parent_id,
...
from ball.data_layer.dl_wallet_store import DataLayerStore
from ball.pools.pool_puzzles import (
...
from ball.util.misc import UInt32Range, UInt64Range, VersionedBlob
from ball.util.path import path_from_root
from ball.util.streamable import Streamable
from ball.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from ball.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, CRCATInfo
from ball.wallet.cat_wallet.cat_utils import CAT_MOD, CAT_MOD_HASH, construct_cat_puzzle, match_cat_puzzle
from ball.wallet.cat_wallet.cat_wallet import CATWallet
from ball.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from ball.wallet.conditions import Condition, ConditionValidTimes, parse_timelock_info
...
from ball.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
from ball.wallet.derivation_record import DerivationRecord
...
from ball.wallet.did_wallet.did_info import DIDCoinData
from ball.wallet.did_wallet.did_wallet import DIDWallet
from ball.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, match_did_puzzle
from ball.wallet.key_val_store import KeyValStore
from ball.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
from ball.wallet.nft_wallet.nft_wallet import NFTWallet
from ball.wallet.nft_wallet.uncurry_nft import NFTCoinData, UncurriedNFT
from ball.wallet.notification_manager import NotificationManager
from ball.wallet.outer_puzzles import AssetType
from ball.wallet.payment import Payment
from ball.wallet.puzzle_drivers import PuzzleInfo
from ball.wallet.puzzles.clawback.drivers import generate_clawback_spend_bundle, match_clawback_puzzle
from ball.wallet.puzzles.clawback.metadata import ClawbackMetadata, ClawbackVersion
from ball.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_hash_for_synthetic_public_key,
)
from ball.wallet.puzzles.stake.drivers import match_stake_puzzle, generate_stake_spend_bundle
from ball.wallet.puzzles.stake.metadata import StakeMetadata, StakeVersion
from ball.wallet.sign_coin_spends import sign_coin_spends
from ball.wallet.singleton import create_singleton_puzzle, get_inner_puzzle_from_singleton, get_singleton_id_from_puzzle
from ball.wallet.trade_manager import TradeManager
from ball.wallet.trading.trade_status import TradeStatus
from ball.wallet.transaction_record import TransactionRecord, TransactionRecordOld
...
from ball.wallet.util.puzzle_decorator import PuzzleDecoratorManager
from ball.wallet.util.query_filter import HashFilter
from ball.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from ball.wallet.util.tx_config import TXConfig, TXConfigLoader
from ball.wallet.util.wallet_sync_utils import (
    PeerRequestException,
    fetch_coin_spend_for_coin_state,
    last_change_height_cs,
)
from ball.wallet.util.wallet_types import CoinType, WalletIdentifier, WalletType
from ball.wallet.vc_wallet.cr_cat_drivers import CRCAT, ProofsChecker, construct_pending_approval_state
from ball.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from ball.wallet.vc_wallet.vc_drivers import VerifiedCredential
from ball.wallet.vc_wallet.vc_store import VCStore
from ball.wallet.vc_wallet.vc_wallet import VCWallet
from ball.wallet.wallet import Wallet
from ball.wallet.wallet_blockchain import WalletBlockchain
from ball.wallet.wallet_coin_record import MetadataTypes, WalletCoinRecord
from ball.wallet.wallet_coin_store import WalletCoinStore
from ball.wallet.wallet_info import WalletInfo
from ball.wallet.wallet_interested_store import WalletInterestedStore
from ball.wallet.wallet_nft_store import WalletNftStore
from ball.wallet.wallet_pool_store import WalletPoolStore
from ball.wallet.wallet_protocol import WalletProtocol
from ball.wallet.wallet_puzzle_store import WalletPuzzleStore
from ball.wallet.wallet_retry_store import WalletRetryStore
from ball.wallet.wallet_transaction_store import WalletTransactionStore
from ball.wallet.wallet_user_store import WalletUserStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSBallConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def spend_clawback_coins(
        ...
        for coin, metadata in clawback_coins.items():
            try:
                ...
                # Remove the clawback hint since it is unnecessary for the BALL coin
        ...
        if fee > 0:
            ball_tx = await self.main_wallet.create_tandem_ball_tx(
                fee, tx_config, Announcement(coin_spends[0].coin.name(), message)
            )
            assert ball_tx.spend_bundle is not None
            spend_bundle = SpendBundle.aggregate([spend_bundle, ball_tx.spend_bundle])
        assert derivation_record is not None
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        ball_spam_amount = self.config.get("ball_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if ball_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(ball_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < ball_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSBallConnection) -> Optional[bytes32]:
        # Get minter DID
