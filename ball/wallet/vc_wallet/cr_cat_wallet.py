...
from ball.wallet.cat_wallet.cat_info import CATCoinData, CRCATInfo
from ball.wallet.cat_wallet.cat_utils import CAT_MOD_HASH, CAT_MOD_HASH_HASH, construct_cat_puzzle
...
from ball.wallet.util.tx_config import TXConfig
...
from ball.wallet.vc_wallet.cr_cat_drivers import (
    CRCAT,
    CRCATMetadata,
    CRCATVersion,
    ProofsChecker,
...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSBallConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
            ...
            if first:
                ...
                if need_ball_transaction:
                    if fee > regular_ball_to_claim:
                        ...
                        ball_tx, ball_announcement = await self.create_tandem_ball_tx(
                            fee,
                            uint64(regular_ball_to_claim),
                            tx_config,
                        )
                        assert ball_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
                            coin_announcements_to_assert={ball_announcement.name()},
                            conditions=extra_conditions,
                        )
                else:
                    innersol = self.standard_wallet.make_solution(
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [ball_tx.spend_bundle]
            ...
            [*vc_txs, *([ball_tx] if ball_tx is not None else [])],
        )
    ...
    async def claim_pending_approval_balance(
        ...
        # Select the relevant BALL coins
        if fee > 0:
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(ball_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if ball_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, ball_tx.spend_bundle])
        else:
            ball_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(ball_tx, spend_bundle=None),) if ball_tx is not None else []),
        ]
