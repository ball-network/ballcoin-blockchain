...
from ball.wallet.puzzle_drivers import Solver
...
from ball.wallet.vc_wallet.cr_cat_drivers import CRCAT, CRCATSpend, ProofsChecker, construct_pending_approval_state
from ball.wallet.vc_wallet.vc_drivers import VerifiedCredential
from ball.wallet.vc_wallet.vc_store import VCProofs, VCRecord, VCStore
...
if TYPE_CHECKING:
    from ball.wallet.wallet_state_manager import WalletStateManager  # pragma: no cover
...
class VCWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSBallConnection, coin_data: Optional[Streamable]
    ) -> None:
        """
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if fee > 0:
            ...
            ball_tx = await self.wallet_state_manager.main_wallet.create_tandem_ball_tx(
        ...
        if ball_tx is not None and ball_tx.spend_bundle is not None:
            spend_bundles.append(ball_tx.spend_bundle)
            tx_list.append(dataclasses.replace(ball_tx, spend_bundle=None))
    ...
    async def revoke_vc(
        ...
        if fee > 0:
            ball_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.create_tandem_ball_tx(
                fee, tx_config, vc_announcement
            )
            assert did_tx.spend_bundle is not None
            assert ball_tx.spend_bundle is not None
            new_bundle = SpendBundle.aggregate([ball_tx.spend_bundle, did_tx.spend_bundle])
