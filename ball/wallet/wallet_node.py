...
from ball.consensus.blockchain import ReceiveBlockResult
from ball.consensus.constants import ConsensusConstants
from ball.daemon.keychain_proxy import (
    KeychainProxy,
...
from ball.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
from ball.util.errors import KeychainIsLocked, KeychainProxyConnectionFailure, KeychainIsEmpty, KeychainKeyNotFound
...
from ball.wallet.wallet_weight_proof_handler import get_wp_fork_point, WalletWeightProofHandler
...
class WalletNode:
    ...
    def on_disconnect(self, peer: WSBallConnection):
    ...
    async def on_connect(self, peer: WSBallConnection):
        if self._wallet_state_manager is None:
            return None
    ...
    async def is_peer_synced(
        self, peer: WSBallConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    def get_full_node_peer(self) -> Optional[WSBallConnection]:
        """
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSBallConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSBallConnection):
        if self._wallet_state_manager is None:
                ...
                async with self.wallet_state_manager.lock:
                    ...
                    if peak_hb is None or new_peak.weight > peak_hb.weight:
                        backtrack_fork_height: int = await self.wallet_short_sync_backtrack(
                            header_block, difficulty_coefficient, peer
                        )
                    else:
    ...
    async def wallet_short_sync_backtrack(
        ...
        for block, difficulty_coefficient in blocks:
            ...
            res, err = await self.wallet_state_manager.blockchain.receive_block(block, difficulty_coefficient)
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSBallConnection, peak: HeaderBlock
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSBallConnection) -> CoinSpend:
