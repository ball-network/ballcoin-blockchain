...
from ball.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from ball.protocols import farmer_protocol, wallet_protocol
...
from ball.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from ball.wallet.settings.settings_objects import BackupInitialized
...
from ball.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    async def update_stakings(self, peer: WSBallConnection, height: uint64, farmer_public_key: G1Element) -> None:
        "fetch staking"
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSBallConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSBallConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSBallConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSBallConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSBallConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
