...
PLOT_PUBLIC_KEY_CACHE: LRUCache[bytes, G1Element] = LRUCache(50000)
...
def generate_plot_public_key(local_pk: G1Element, farmer_pk: G1Element, include_taproot: bool = False) -> G1Element:
    key = bytes(local_pk + farmer_pk)
    plot_public_key: Optional[G1Element] = PLOT_PUBLIC_KEY_CACHE.get(key)
    if plot_public_key is None:
        if include_taproot:
            taproot_sk: PrivateKey = generate_taproot_sk(local_pk, farmer_pk)
            plot_public_key = local_pk + farmer_pk + taproot_sk.get_g1()
        else:
            plot_public_key = local_pk + farmer_pk
        PLOT_PUBLIC_KEY_CACHE.put(key, plot_public_key)
