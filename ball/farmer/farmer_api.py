...
from ball.farmer.farmer import Farmer, increment_pool_stats, strip_old_entries
from ball.full_node.full_node_api import FullNodeAPI
from ball.harvester.harvester_api import HarvesterAPI
from ball.protocols import farmer_protocol, harvester_protocol
from ball.protocols.farmer_protocol import DeclareProofOfSpace, SignedValues
from ball.protocols.harvester_protocol import (
    PlotSyncDone,
    PlotSyncPathList,
    PlotSyncPlotList,
    PlotSyncStart,
...
from ball.protocols.pool_protocol import (
    PoolErrorCode,
    PostPartialPayload,
    PostPartialRequest,
...
from ball.server.server import ssl_context_for_root
...
from ball.types.stake_value import STAKE_PER_COEFFICIENT
from ball.util.api_decorators import api_request
from ball.util.ints import uint8, uint16, uint32, uint64
...
class FarmerAPI:
    ...
    async def new_proof_of_space(
        self, new_proof_of_space: harvester_protocol.NewProofOfSpace, peer: WSBallConnection
    ) -> None:
        """
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            headers={"User-Agent": f"BallCoin Blockchain v.{__version__}"},
                        ) as resp:
                            if not resp.ok:
    ...
    async def new_signage_point(
            self, new_signage_point: farmer_protocol.NewSignagePoint, peer: WSBallConnection
    ) -> None:
    ...
    @api_request(peer_required=True)
    async def farming_info(self, request: farmer_protocol.FarmingInfo, peer: WSBallConnection) -> None:
        self.farmer.state_changed(
            "new_farming_info",
            {
                "farming_info": {
    ...
    @api_request(peer_required=True)
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: WSBallConnection) -> None:
        self.farmer.log.warning(f"Respond plots came too late from: {peer.get_peer_logging()}")

    @api_request(peer_required=True)
    async def plot_sync_start(self, message: PlotSyncStart, peer: WSBallConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].sync_started(message)

    @api_request(peer_required=True)
    async def plot_sync_loaded(self, message: PlotSyncPlotList, peer: WSBallConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_loaded(message)

    @api_request(peer_required=True)
    async def plot_sync_removed(self, message: PlotSyncPathList, peer: WSBallConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_removed(message)

    @api_request(peer_required=True)
    async def plot_sync_invalid(self, message: PlotSyncPathList, peer: WSBallConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_invalid(message)

    @api_request(peer_required=True)
    async def plot_sync_keys_missing(self, message: PlotSyncPathList, peer: WSBallConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_keys_missing(message)

    @api_request(peer_required=True)
    async def plot_sync_duplicates(self, message: PlotSyncPathList, peer: WSBallConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_duplicates(message)

    @api_request(peer_required=True)
    async def plot_sync_done(self, message: PlotSyncDone, peer: WSBallConnection) -> None:
