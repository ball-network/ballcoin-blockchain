...
from ball.plot_sync.delta import Delta
from ball.plot_sync.receiver import Receiver
from ball.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from ball.server.server import BallServer, ssl_context_for_root
...
from ball.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from ball.util.errors import KeychainProxyConnectionFailure
...
from ball.util.logging import TimedDuplicateFilter
...
from ball.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'ball keys generate' or open the UI."
        try:
        ...
        if "ball_target_address" not in self.config:
            self.config = config["farmer"]
        if "ball_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "ball_target_address" not in self.config or "ball_target_address" not in self.pool_config:
            log.debug("ball_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["ball_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["ball_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]) -> None:
        with lock_and_load_config(self._root_path, "config.yaml") as config:
            if farmer_target_encoded is not None:
                ...
                config["farmer"]["ball_target_address"] = farmer_target_encoded
            if pool_target_encoded is not None:
                self.pool_target_encoded = pool_target_encoded
                self.pool_target = decode_puzzle_hash(pool_target_encoded)
                config["pool"]["ball_target_address"] = pool_target_encoded
