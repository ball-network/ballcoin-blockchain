...
from ball.protocols.protocol_state_machine import message_requires_reply
from ball.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
from ball.protocols.shared_protocol import protocol_version
from ball.server.introducer_peers import IntroducerPeers
from ball.server.outbound_message import Message, NodeType
from ball.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from ball.util.network import is_localhost, is_in_network
from ball.util.ssl_check import verify_ssl_certs_and_keys
...
class BallServer:
    def __init__(
        ...
        ball_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSBallConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSBallConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.ball_ca_crt_path, self.ball_ca_key_path = ball_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSBallConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.ball_ca_crt_path, self.ball_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSBallConnection] = None
        try:
            connection = WSBallConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSBallConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.ball_ca_crt_path, self.ball_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSBallConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSBallConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSBallConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSBallConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSBallConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSBallConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSBallConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def is_trusted_peer(self, peer: WSBallConnection, trusted_peers: Dict) -> bool:
