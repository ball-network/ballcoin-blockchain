...
from ball.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
...
from ball.util.network import WebServer, is_in_network, is_localhost
...
class BallServer:
    def __init__(
        ...
        ball_ca_crt_path, ball_ca_key_path = ball_ca_crt_key
        ...
        if self._local_type in authenticated_client_types:
            ...
            self.ssl_client_context = ssl_context_for_client(
                ball_ca_crt_path, ball_ca_key_path, public_cert_path, public_key_path
            )
        ...
        if self._local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            self.ssl_context = ssl_context_for_server(
                ball_ca_crt_path, ball_ca_key_path, public_cert_path, public_key_path, log=self.log
            )
    ...
    async def start_server(self, prefer_ipv6: bool, on_connect: Callable = None):
        if self.webserver is not None:
            raise RuntimeError("BallServer already started")
