...
from ball.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
...
from ball.server.ws_connection import ConnectionCallback, WSBallConnection
...
from ball.util.network import WebServer, is_in_network, is_localhost, is_trusted_peer
...
class BallServer:
    ...
    all_connections: Dict[bytes32, WSBallConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=ball_ca_crt_path,
                ca_key=ball_ca_key_path,
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        try:
            connection = WSBallConnection.create(
    ...
    async def connection_added(
        self, connection: WSBallConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSBallConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    def is_trusted_peer(self, peer: WSBallConnection, trusted_peers: Dict[str, Any]) -> bool:
