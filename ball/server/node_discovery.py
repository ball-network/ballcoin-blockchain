...
from ball.protocols import full_node_protocol, introducer_protocol
...
from ball.server.outbound_message import NodeType, make_msg, Message
from ball.server.peer_store_resolver import PeerStoreResolver
from ball.server.server import BallServer
from ball.types.peer_info import PeerInfo, TimestampedPeerInfo
...
class FullNodeDiscovery:
    ...
    async def on_connect(self, peer: ws.WSBallConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSBallConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSBallConnection):
    ...
    async def on_connect_callback(self, peer: ws.WSBallConnection):
