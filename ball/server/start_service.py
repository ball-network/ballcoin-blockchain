...
class Service(Generic[_T_RpcServiceProtocol, _T_ApiProtocol]):
    def __init__(
        ...
        *,
        ...
        ball_ca_crt, ball_ca_key = ball_ssl_ca_paths(root_path, self.config)
    ...
    async def manage(self, *, start: bool = True) -> AsyncIterator[None]:
        ...
        async with contextlib.AsyncExitStack() as async_exit_stack:
            try:
                ...
                self._log.info("Waiting for BallServer to be closed")
    ...
    async def setup_process_global_state(self, signal_handlers: SignalHandlers) -> None:
        ...
        proctitle_name = f"ball_{self._service_name}"
    ...
    def _accept_signal(
        ...
        if callable(getattr(signal_, "name")):
            self._log.info("Received signal %s (%s), %s.", signal_.name, signal_.value, message)
        else:
            self._log.info("Received signal %s, %s.", signal_, message)
