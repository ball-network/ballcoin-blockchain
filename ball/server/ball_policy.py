...
class BallSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _ball_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class BallProactor(IocpProactor):
        ...
        async def _ball_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._ball_accept(listener)
                except WindowsError as exc:  # pylint: disable=E0602
                    if exc.winerror not in (_winapi.ERROR_NETNAME_DELETED, _winapi.ERROR_OPERATION_ABORTED):
                        raise

        def _ball_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: BallProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._ball_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class BallProactorEventLoop(ProactorEventLoop):
...
class BallPolicy(asyncio.DefaultEventLoopPolicy):
...
def set_ball_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(BallPolicy())
