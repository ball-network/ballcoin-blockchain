...
from ball.consensus.blockchain import Blockchain, ReceiveBlockResult
...
from ball.util.db_wrapper import DBWrapper
...
class FullNode:
    ...
    def set_server(self, server: BallServer):
        self.server = server
        dns_servers = []
        try:
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSBallConnection):
        """
    ...
    def remove_subscriptions(self, peer: ws.WSBallConnection):
    ...
    async def sync_from_fork_point(
        ...
        async def fetch_block_batches(batch_queue, peers_with_peak: List[ws.WSBallConnection]):
            try:
        ...
        batch_queue: asyncio.Queue[Tuple[ws.WSBallConnection, List[FullBlock]]] = asyncio.Queue(
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSBallConnection):
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSBallConnection):
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSBallConnection):
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
