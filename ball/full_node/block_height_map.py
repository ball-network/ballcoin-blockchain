...
@streamable
@dataclass(frozen=True)
class CFCache(Streamable):
    content: List[Tuple[uint32, uint104]]
...
class BlockHeightMap:
    ...
    # the file we're saving coefficient cache to
    __height_to_coefficient_filename: Path
    ...
    async def create(cls, blockchain_dir: Path, db: DBWrapper2) -> "BlockHeightMap":
        ...
        self.__dirty = 0
        self.__dirty_coefficient = 0
        self.__height_to_hash = bytearray()
        self.__sub_epoch_summaries = {}
        self.__height_to_coefficient = {}
        self.__height_to_hash_filename = blockchain_dir / "height-to-hash"
        self.__ses_filename = blockchain_dir / "sub-epoch-summaries"
        self.__height_to_coefficient_filename = blockchain_dir / "height-to-coefficient"

        try:
            async with aiofiles.open(self.__height_to_coefficient_filename, "rb") as f:
                self.__height_to_coefficient = {k: Decimal(format(v / 10 ** 28, '.28f')) for (k, v) in
                                                CFCache.from_bytes(await f.read()).content}
        except Exception:
    ...
    def set_coefficient(self, height: uint32, coefficient: Decimal) -> None:
        ...
        self.__dirty_coefficient += 1

    def get_coefficient(self, height: uint32) -> Optional[Decimal]:
        if height in self.__height_to_coefficient:
            return self.__height_to_coefficient[height]
        return None

    def del_coefficient(self, height: uint32):
        if height in self.__height_to_coefficient:
            del self.__height_to_coefficient[height]
            self.__dirty_coefficient += 1
            self.maybe_flush_coefficient()

    def contains_coefficient(self, height: uint32) -> bool:
        return height in self.__height_to_coefficient

    async def maybe_flush_coefficient(self) -> None:
        if self.__dirty_coefficient < 32:
            return

        coefficient_buf = bytes(
            CFCache([(k, uint104(v * 10 ** 28)) for (k, v) in self.__height_to_coefficient.items()]))
