...
ROOT = pathlib.Path(importlib.import_module("ball").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `ball` file name (which is the executable)
    and the `ball` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `ball` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("ballcoin-blockchain"))[0]
...
# TODO: collapse all these entry points into one `ball_exec` entrypoint that accepts the server as a parameter

entry_points = ["ball.cmds.ball"] + [f"ball.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "ball.util.bip39"])
...
datas.append((f"{ROOT}/ball/util/english.txt", "ball/util"))
datas.append((f"{ROOT}/ball/util/initial-config.yaml", "ball/util"))
datas.append((f"{ROOT}/ball/wallet/puzzles/*.hex", "ball/wallet/puzzles"))
datas.append((f"{ROOT}/ball/ssl/*", "ball/ssl"))
...
add_binary("ball", f"{ROOT}/ball/cmds/ball.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/ball/daemon/server.py", COLLECT_ARGS)
