...
get_madmax_filename() {
  ...
  export BALL_PLOT="ball_plot"
  if [ "$KSIZE" = "k34" ]; then
    export BALL_PLOT="ball_plot_k34"
  fi
  SUFFIX=""
  if [ "$OS" = "macos" ]; then
    if [ "$ARCH" = "arm64" ]; then
  ...
  echo "${BALL_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
